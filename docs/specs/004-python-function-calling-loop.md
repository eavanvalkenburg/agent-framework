...
| Approved streaming resume | Result update precedes final text and final response matches non-streaming shape. | `test_approval_resume_returns_result_without_mutating_inputs[streaming-approved]` |
| Rejected streaming resume | Rejection result update precedes final text and tool executes zero times. | `test_approval_resume_returns_result_without_mutating_inputs[streaming-rejected]` |
...
| Approval-time user input | Every user-input request from one approved execution returns in order with assistant role and no extra model call. | `packages/core/tests/core/test_harness_tool_approval.py::test_approval_resume_returns_all_user_input_requests_without_another_model_call` |
...
| Shared call budget | Auto-approved re-entry does not reset `max_function_calls`. | `test_tool_approval_middleware_auto_approved_loops_share_function_call_budget` |
...
| Consecutive error cap | Error threshold stops repeated failures and submits collected results. | `test_function_invocation_config_max_consecutive_errors`, `test_streaming_function_invocation_config_max_consecutive_errors`, `test_approval_resume_error_limit_forces_final_no_tool_response` |
