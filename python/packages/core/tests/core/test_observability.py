...
async def test_agent_invoke_span_aggregates_usage_across_tool_calls(span_exporter: InMemorySpanExporter):
    ...
    assert chat_spans[0].attributes.get(OtelAttr.INPUT_TOKENS) == 2239  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    assert chat_spans[0].attributes.get(OtelAttr.OUTPUT_TOKENS) == 192  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    assert chat_spans[1].attributes.get(OtelAttr.INPUT_TOKENS) == 2569  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    assert chat_spans[1].attributes.get(OtelAttr.OUTPUT_TOKENS) == 99  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]

    # The invoke_agent span must report the aggregate across all LLM round-trips
    assert agent_span.attributes.get(OtelAttr.INPUT_TOKENS) == 2239 + 2569  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
    assert agent_span.attributes.get(OtelAttr.OUTPUT_TOKENS) == 192 + 99  # type: ignore[union-attr]  # ty: ignore[unresolved-attribute]
