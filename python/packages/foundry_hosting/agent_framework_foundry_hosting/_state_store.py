...
def _store_scope(config: AgentConfig, platform_context: FoundryAgentRequestContext) -> FoundryRequestScope | None:
    if not config.is_hosted:
        return None
    if not platform_context.session_id:
        raise RuntimeError("Foundry hosted state storage requires a platform session ID.")
    ...
    def __init__(
        ...
        *,
        ...
        Args:
            ...
            scope: Trusted Foundry request scope for sandbox-specific checkpoint storage.
        """
        if not context_id:
...
class FoundryAgentSessionStore(SessionStore):
    ...
    def __init__(
        self, platform_context: FoundryAgentRequestContext, *, scope: FoundryRequestScope | None = None
    ) -> None:
        self.platform_context = platform_context
        self._scope = scope
    ...
    async def set(self, session_id: str, session: AgentSession) -> None:
        ...
        async with store:
            try:
                if session_id in self._etags and self._etags[session_id] is None:
                    result = await store.create_item(
                        session_id, session.to_dict(), call_id=self.platform_context.call_id
                    )
                elif session_id in self._etags:
                    result = await store.set_item(
                        session_id,
                        session.to_dict(),
                        if_match=self._etags[session_id],
                        call_id=self.platform_context.call_id,
                    )
                else:
                ...
                raise RuntimeError("Another request advanced this agent session.") from exc
