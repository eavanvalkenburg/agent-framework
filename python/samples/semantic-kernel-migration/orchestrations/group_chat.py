...
async def run_agent_framework_example(task: str) -> str:
    ...
    researcher = Agent(
        ...
        client=OpenAIChatClient(backend="azure_openai", credential=credential),
    )
    ...
    workflow = GroupChatBuilder(
        ...
        orchestrator_agent=OpenAIChatClient(backend="azure_openai", credential=credential).as_agent(),
    ).build()
